...
from local_settings import model, model_name
import logging
...
ada_one_agent = Agent(
    name="AddoneAgent",
    instructions="你是一个专门将输入数字加1的代理，使用add_one工具。返回你做了什么，以及结果",
    tools=[add_one],
    model=model_name,
)
subtract_one_agent = Agent(
    name="SubtractoneAgent",
    instructions="你是一个专门将输入数字减1的代理，使用subtract_one工具。返回你做了什么，以及结果",
    tools=[subtract_one],
    model=model_name,
)
multiply_by_two_agent = Agent(
    name="MultiplyByTwoAgent",
    instructions="你是一个专门将输入数字乘以2的代理，使用multiply_by_two工具。返回你做了什么，以及结果",
    tools=[multiply_by_two],
    model=model_name,
)
divide_by_two_agent = Agent(
    name="DivideByTwoAgent",
    instructions="你是一个专门将输入数字除以2的代理，使用divide_by_two工具。返回你做了什么，以及结果",
    tools=[divide_by_two],
    model=model_name,
)
...
triage_agent = Agent(
    name="TriageAgent",
    instructions="你是一个调度代理，负责将当前数字转换为目标数字（假设数字均为整数）。输入相"
                 "你的任务是：\n"
                 "1. 分析当前数字与目标数字的差值，推理哪种操作（加1、减1、乘2、除2）步骤最少的接近目标数字，根据\n"
                 "2. 考虑操作历史，避免无限循环（如反复加1和减1），尽量用最少的操作步骤完成目标。\n"
                 "3. 当任务完成时输出：exit",
    # tools=[add_one, subtract_one, multiply_by_two, divide_by_two],
...
async def transform_number(initial_number: float, target_number: float):
    ...
    while True:
        ...
        if turn >= max_turns:
            raise ValueError(
                f"在 {max_turns} 轮次内未能达到目标 {target_number}。最终数字：{current_number}"
            )
        
        # 准备调度代理的输入
        result = await Runner.run_streamed(triage_agent, input=history, max_turns=max_turns)
